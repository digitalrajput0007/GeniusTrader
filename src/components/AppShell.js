...
const PerformanceIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6m3 6V5m3 14v-8m4 8V3M6 19v-4" /></svg>;
      ...
      <aside className={`fixed inset-y-0 left-0 bg-primary-light p-4 flex flex-col justify-between border-r border-gray-700 w-64 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 ease-in-out z-30`}>
        <div>
          ...
          <nav>
            ...
            <NavLink to="/performance" className={({ isActive }) => `${navLinkClasses} ${isActive ? activeNavLinkClasses : ''}`} onClick={() => setIsSidebarOpen(false)}><PerformanceIcon /><span className="ml-4">Performance</span></NavLink>
